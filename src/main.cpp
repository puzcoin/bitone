...
#if defined(NDEBUG)
#error "BONE cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uBONE) are considered zero fee (for relaying and mining)
...
const string strMessageMagic = "Bone Signed Message:\n";
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : xBONE with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : xBONE spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: xBONE spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight >2  && nHeight <= 10) {
        nSubsidy = 5000 * COIN;
    } else if (nHeight > 10 && nHeight <= 110 ) {
        nSubsidy = 100 * COIN;
    } else if (nHeight > 110 && nHeight <= 200){
        nSubsidy = 10 * COIN;
    }
    return nSubsidy;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from BONE
...
void RecalculateXIONSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from xBONE supply
...
bool RecalculateBONESupply(int nHeightStart)
{
...
/** FornaxA: Bone accepts PoS during PoW phase 
 *
 *   if (pindex->nHeight <= Params().LAST_POW_BLOCK() && block.IsProofOfStake())
 *       return state.DoS(100, error("ConnectBlock() : PoS period not active"),
 *           REJECT_INVALID, "PoS-early");
 */
/** cevap: Bone accepts PoS during PoW phase, PoW Ends with block 1000, PoS starts with 455, testnet 74 
 *
 */ 
/** cevap: Bone accepts PoS during PoW phase, PoW Ends with block 1000, PoS starts with 455, testnet 74 
 *
 ...
 */
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateBONESupply(Params().Zerocoin_StartHeight());
    }

    //Track xBONE money supply in the block index
    if (!UpdateXIONSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new xBONE supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s xBONESpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert Bone to xBONE
...
/*    if (block.GetBlockTime() > Params().Zerocoin_StartTime()) {
        ...
        // BONETor - disable reject block as our blocks are in version 8 since block 1
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent xBONE spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of xBONE serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d xBONE mints and %d xBONE spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BONE: We use certain sporks during IBD, so check to see if they are
