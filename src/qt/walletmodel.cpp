...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered bone address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_3_SWIFTTX_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BONE.").arg(GetSporkValue(SPORK_3_SWIFTTX_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal bone:URI (bone:XyZ...?message=example)
            {
...
static void NotifyxBONEReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyxBONEReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyxBONEReset.connect(boost::bind(NotifyxBONEReset, this));
}
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyxBONEReset.disconnect(boost::bind(NotifyxBONEReset, this));
}
