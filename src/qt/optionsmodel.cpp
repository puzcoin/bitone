...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BONE);
    ...
    if (!settings.contains("nAnonymizeBONEAmount"))
        settings.setValue("nAnonymizeBONEAmount", 1000);

    nAnonymizeBONEAmount = settings.value("nAnonymizeBONEAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBONEAmount"))
        SoftSetArg("-anonymizeionamount", settings.value("nAnonymizeBONEAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bone.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBONEAmount:
            return QVariant(nAnonymizeBONEAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBONEAmount:
            nAnonymizeBONEAmount = value.toInt();
            settings.setValue("nAnonymizeBONEAmount", nAnonymizeBONEAmount);
            emit anonymizeBONEAmountChanged(nAnonymizeBONEAmount);
            break;
        case CoinControlFeatures:
